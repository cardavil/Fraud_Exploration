...
Esquema completo de Postgres (Supabase): las 8 tablas del seed generadas por `supabase/generate_seed.py` + las 3 tablas operativas del copiloto (`supabase/rate_limit.sql`, `supabase/audit.sql`), con DDL real, quien escribe cada cosa, reglas de integridad y limites.
...
| Principio | Detalle |
|---|---|
| Single-writer | TODO lo escribe el seed o el sistema: el seed puebla las 8 tablas de datos, la RPC `copilot_hit` escribe los contadores, la Edge Function escribe el audit. El visitante NUNCA escribe nada |
...
| `copilot_usage` | 1/dia | rpc | ninguno |
| `copilot_ip_usage` | efimera | rpc | ninguno |
| `copilot_audit` | 5/run | copilot | ninguno |
...
| Columna | Tipo | Escribe | Descripcion |
|---|---|---|---|
| `account_id` | text | seed | PK, formato `ACC#####` (validado `^ACC\d{5}$` en el copiloto) |
...
| `full_name` | text | seed | Copia denormalizada — el copiloto NUNCA la selecciona (TRD §6) |
...
Contador global diario del copiloto (cap 250/dia, TRD §7). PK `day`; escribe SOLO la RPC `copilot_hit`.
...
REVOKE ALL ON copilot_usage, copilot_ip_usage FROM anon, authenticated;
-- No policies on purpose: only the service role (bypasses RLS) touches them.

CREATE OR REPLACE FUNCTION copilot_hit(client_ip text, max_per_min integer, max_per_day integer)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  day_ok boolean;
BEGIN
  INSERT INTO copilot_ip_usage AS i (ip, minute, hits)
  ...
  INSERT INTO copilot_usage AS u (day, hits)
  ...
  DELETE FROM copilot_ip_usage WHERE minute < now() - interval '1 day';

  RETURN ip_ok AND day_ok;
END;
$$;
REVOKE EXECUTE ON FUNCTION copilot_hit(text, integer, integer) FROM PUBLIC, anon, authenticated;
```

## 11. copilot_ip_usage

Contador por IP y minuto (limite 8/min/IP, TRD §7). PK compuesta `(ip, minute)`; escribe SOLO la RPC `copilot_hit`, que ademas borra filas de mas de 1 dia en cada llamada — tabla autolimpiante.
...
```sql
CREATE TABLE IF NOT EXISTS copilot_ip_usage (
  ip     text NOT NULL,
  minute timestamptz NOT NULL,
  hits   integer NOT NULL DEFAULT 0,
  PRIMARY KEY (ip, minute)
);
ALTER TABLE copilot_ip_usage ENABLE ROW LEVEL SECURITY;
```

## 12. copilot_audit

Traza por agente por run del pipeline (TRD §8): 5 filas por ejecucion. PK `id` (identity); escribe SOLO la Edge Function `copilot` (service role) via `persistAudit`.

| Columna | Tipo | Escribe | Descripcion |
|---|---|---|---|
| `id` | bigint identity | copilot | PK autogenerada |
| `run_id` | uuid | copilot | Agrupa las 5 filas de un run (`crypto.randomUUID()`); indexado |
| `account_id` | text | copilot | Cuenta analizada |
| `agent` | text | copilot | Uno de los 5 agentes (TRD §4) |
| `model_used` | text | copilot | Modelo que respondio; NULL si fallaron los 3 de la cadena |
| `attempts` | integer | copilot | Intentos acumulados en el wrapper (retry + fallbacks) |
| `fallback_used` | boolean | copilot | true si respondio un modelo distinto al preferido (o fallo total) |
| `latency_ms` | integer | copilot | Latencia total del wrapper para ese agente |
| `ok` | boolean | copilot | El agente produjo salida valida; el reviewer con ok=false marca un final degradado (TRD §4) |
| `ts` | timestamptz | copilot | DEFAULT now() |
...
copilot_usage / copilot_ip_usage / copilot_audit: sin FKs — operativas, aisladas del dataset
```
...
| 3 tablas operativas | ON | NINGUNA (a proposito) | `REVOKE ALL` — nada; solo el service role las toca. `EXECUTE` de `copilot_hit` tambien revocado de PUBLIC/anon/authenticated |
